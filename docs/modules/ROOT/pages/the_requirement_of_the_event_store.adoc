...
* *To retry purpose.*
+
Any kind of execution can retry if the execution is failed due to a xref:retryable_executor_exception.adoc[RetryableExecutorException] like network issues.
...
+
StackSaga offers a dashboard called xref:stacksaga-admin:stacksaga-cloud-window.adoc[Stacksaga Cloud-Window], which allows you to monitor transaction tracing details and review any errors related to transactions.
