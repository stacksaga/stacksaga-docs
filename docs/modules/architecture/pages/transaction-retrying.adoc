...
IMPORTANT: Sometimes you might think that why cannot be re-invoked the transactions by the own instance without an agent? +
...
After a while, the scheduler is triggered for replying transactions on each instance.
