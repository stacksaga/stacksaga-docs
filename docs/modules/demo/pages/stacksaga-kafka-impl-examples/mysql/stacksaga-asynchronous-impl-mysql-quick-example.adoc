...
. The target *worker* (`user-service`) consumes the command through its `@SagaEndpoint`, runs the business logic, and publishes a *reply* to the framework-managed per-domain callback topic.
...
* `domainCallbackTopicSuffix` Suffix for creating the domain's callback topic. +
read more xref:implementations:stacksaga-kafka-implementation/orchestrator/properties.adoc#topic_model_stacksaga_kafka_orchestrator[]

NOTE: Not shown here since `SHARED_GLOBAL` doesn't need it, but `SHARED_GROUP`/`ISOLATED` event managers also configure `autoStart` (whether the container starts automatically on application startup, default `true`) via the `sharedGroupExecutionListener`/`isolatedExecutionListener` attributes — see xref:implementations:stacksaga-kafka-implementation/orchestrator/properties.adoc#orchestrator_auto_start[].
...
<4> Override the `onNext` method to define the routing logic of the event manager. This method is invoked by the framework after each primary-execution topic (span/atomic execution) completes successfully until the end of the transaction (if it has not failed with `NonRetryableExecutorException`). You use the given parameters and the `actionUtil` helper to return the next navigation action (`actionUtil.next(...)`, `actionUtil.complete()`, or `actionUtil.error(...)`).
<5> If the most recently completed topic is `DO_USER_VALIDATED`, route the engine to `DO_ORDER_RESERVED` next via `actionUtil.next(...)`.
<6> If the most recently completed topic is `DO_ORDER_RESERVED`, route the engine to `DO_MAKE_PAYMENT` next via `actionUtil.next(...)`.
<7> `DO_MAKE_PAYMENT` is the *last* span, so once it completes there is nothing more to route — return `actionUtil.complete()` to finish the saga successfully and move it to the terminal `COMPLETED` state. +
...
Returning `actionUtil.error(...)` (or throwing any exception) from `onNext()` transitions the saga to `FAILED` and triggers compensation.
...
<2> The `PlaceOrderHandler` class implements the `KafkaTransactionEventListener` interface, which lets it react to the transaction events the StackSaga engine emits during the flow. xref:implementations:stacksaga-kafka-implementation/orchestrator/orchestrator-usage.adoc#event_listener_for_stage_change_events[read more] +
NOTE: The listener comes in a blocking and a non-blocking flavour — use `KafkaTransactionEventListener` in a servlet (MVC) application and `ReactiveKafkaTransactionEventListener` in a reactive (WebFlux) application.
...
* `topicNameSuffix`: provide the suffix of the topic name for this endpoint. the framework will create the real topic name by prefixing the provided suffix with `saga.` — so `user-service.validate-user` becomes the real topic `saga.user-service.validate-user` (if your suffix already starts with `saga.`, it is kept as-is). this is the topic the `EventManager` sends command messages to, to trigger the execution of this endpoint.
...
* `autoStart`: not shown here since it defaults to `true`; controls whether this endpoint's listener container starts automatically on application startup. Every `@SagaEndpoint` sharing a `SHARED_GROUP` `containerId` must declare the *same* `autoStart`, or startup fails with a `ValidationException` — see xref:implementations:stacksaga-kafka-implementation/worker/worker-configuration.adoc#worker_auto_start[].
...
The framework prefixes it to build the real topic name — here `order-service.reserve-order` becomes `saga.order-service.reserve-order` (the `saga.` prefix is added only if you don't already include it).
...
* `autoStart`: not shown here since it defaults to `true`; controls whether this endpoint's listener container starts automatically on application startup — see xref:implementations:stacksaga-kafka-implementation/worker/worker-configuration.adoc#worker_auto_start[].
