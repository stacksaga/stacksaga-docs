...
* `listenerScope` The scope for receiving the events from the target service to the domain-response topic. at this example it has been marked `ListenerScope.SHARED` and it does not create a separate receiver for the event manager and use the default shared receiver.
...
* `listenerScope`: The scope for receiving the events from the orchestrator service to this endpoint topics(do/undo). at this example it has been marked `ListenerScope.SHARED` and it does not create a separate receiver for receiving messages and use the shared one.
...
* `listenerScope`: the scope used to receive commands for this endpoint's `do`/`undo` topics. `ListenerScope.SHARED` (used here) reuses a single shared receiver instead of creating a dedicated one.
...
On first start-up the framework auto-creates the required Kafka topics and (via Liquibase) the event-store schema.
