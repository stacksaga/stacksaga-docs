...
Here we create a simple REST controller with an endpoint that triggers the saga by accessing the `SagaTemplate` — the main API for interacting with the StackSaga engine. xref:implementations:stacksaga-sync/orchestrator/stacksaga_template.adoc[read more]

It is an ordinary Spring REST controller with a POST endpoint to place an order. Inside the endpoint we use the `SagaTemplate` to start the flow — providing the initial domain-entity state and specifying the first executor to run.
...
NOTE: `SagaTemplate` is the blocking variant for Spring MVC. In a reactive (WebFlux) application, inject `ReactiveSagaTemplate<DE>` instead. xref:implementations:stacksaga-sync/orchestrator/stacksaga_template.adoc[read more]
...
JDBC connection is used internally by Liquibase for database migration and schema creation.
