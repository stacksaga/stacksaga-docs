...
So, To get started, create a new Spring Boot application and include the following dependencies and plugin configuration in your `pom.xml`:
...
<4> Configuring the `spring-boot-maven-plugin` with the `build-info` goal generates `META-INF/build-info.properties` during the build. StackSaga uses this metadata to record the exact application version at which each transaction was started.

TIP: Adding the `build-info` goal helps you track which version the transaction was started at. If this is not configured, StackSaga cannot determine the application version and records the transaction application version as `unknown` in the event store.
