...
This quick example walks you through building a complete *order-placement* saga with StackSaga's synchronous orchestration, using MySQL as the event-store.
We start with a minimal happy-path implementation and then incrementally add compensation, retrying, and observability.
...
. *Validate User* — validate the user who is placing the order.
. *Reserve Order* — reserve the order inside the orchestrator service.
. *Make Payment* — process the payment to complete the order.

If any step fails, StackSaga automatically *compensates* the previously completed steps in reverse order — for example, releasing the reserved order when the payment fails — so the system is always left in a consistent state.
...
<1> Autowired the `SagaTemplate<DE>` which is the main API for interacting with the StackSaga engine to execute transactions. it provides a fluent API to define and execute the transaction flow. `DE` is the type of the domain entity used in the transaction flow, which is `OrderDomainEntity` in this example.
...
StackSaga has subsystem for handling the retrying. the following are the main steps to add retry capability to the transaction flow:
...
In the previous stages, we have implemented the transaction flow with compensation and retry capabilities. now let's see how we can observe the transaction flow and its events using the trace window provided by StackSaga.
xref:trace-window:stacksaga-trace-window.adoc[] is a web portal provided by StackSaga that allows you to visualize and monitor the transaction flows in secure and user-friendly way.
it provides a graphical representation of the transaction flow, showing the different steps, their statuses, and the events emitted during the transaction execution.
you can use the trace window to gain insights into the transaction behavior, identify any issues or bottlenecks, and monitor the overall health of your transactions.
in this stage, we will see how to access the trace window and use it to observe our transaction flows in real-time.
