...
. State management
. Event publishing
. DomainEntity states serialization and Deserialization
...
Then you know we have to create a domain-entity for storing necessary data.
And also we have to create the executors for each atomic transaction.
Creating domain-entity and executors is not enough to start the execution.
To run the execution, we have to notify SEC with the starting executor and the starting domain-entity.
