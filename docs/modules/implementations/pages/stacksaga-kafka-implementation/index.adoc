...
* *Per-domain reply topology* — outbound command topics are defined per worker action; inbound replies use one reply topic per saga domain.
A system with three saga domains uses exactly three reply topics, keeping Kafka partition management predictable.
