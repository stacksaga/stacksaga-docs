...
:keywords: StackSagaKafkaTemplate, KafkaTransactionEventListener, ReactiveKafkaTransactionEventListener, saga handler, orchestrator configuration
:page-priority: 0.7

= StackSagaKafkaTemplate and Event-Listener

[[stacksaga-kafka-template-usage]]
== StackSagaKafkaTemplate

`StackSagaKafkaTemplate` is the main entry point for accessing the StackSaga-Kafka engine's (SEC) functionalities in the orchestrator service.
...
. Using `StackSagaKafkaTemplate.getCurrentState`
. Using `KafkaTransactionEventListener` or `ReactiveKafkaTransactionEventListener`.

IMPORTANT: using `StackSagaKafkaTemplate.getCurrentState` method to get the current state of the transaction by providing the transactionId approach is not efficient because when you call this method, the engine fetches the current state of the transaction from the event-store.
...
You can implement the `KafkaTransactionEventListener` interface (from package `org.stacksaga.api.listener`) to listen to the transaction events in a non-reactive environment as below.
...
You can implement the `ReactiveKafkaTransactionEventListener` interface (from package `org.stacksaga.api.listener`) to listen to the transaction events in a reactive environment as below.
