...
The topic name is built as `saga.callback.{scope}.{appName}.{domainCallbackTopicSuffix}`, where:

* `{scope}` is `global`, `isolated`, or `shared-group`, matching the event manager's `listenerScope` — so event managers on different scopes never collide on the same topic even if they reuse a `domainCallbackTopicSuffix`.
...
For instance, a `SHARED_GLOBAL` event manager in the `order-service` application with `domainCallbackTopicSuffix = "place-order"` resolves to the real topic `saga.callback.global.order-service.place-order`; the same suffix on an `ISOLATED` event manager instead resolves to `saga.callback.isolated.order-service.place-order`.
