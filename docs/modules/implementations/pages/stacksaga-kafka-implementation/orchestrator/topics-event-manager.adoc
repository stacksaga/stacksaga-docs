...
* Message consumption uses *At-Least-Once* delivery semantics; each message is acknowledged only after it has been processed successfully. If a failure occurs after processing but before the acknowledgement/offset is durably recorded, the message may be redelivered and reprocessed. The framework provides a per-span *idempotency key* on each message so a redelivery can be recognised, but it does *not* discard duplicates automatically — guarding against processing the same reply twice must be handled by idempotent handling logic that uses that key.
