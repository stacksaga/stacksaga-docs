...
public class PlaceOrderEventManager extends AbstractEventManager<OrderDomainEntity, PlaceOrderTopic> { //<1>
    ...
    //<7>
    @Override
    public @NonNull SagaPrimaryEventAction<PlaceOrderTopic> onNext(
...
<7> Override the `onNext()` method to determine the next topic based on the recently completed topic, the current state of the domain entity, and the provided `SagaPrimaryEventActionUtil` (`actionUtil`).
...
Use the `actionUtil` factory parameter to return the routing decision:
* `actionUtil.next(topic)` to route the workflow to the next step/topic.
* `actionUtil.complete()` when all steps have been processed successfully to finish the Long-Running Transaction (LRT).
* `actionUtil.error(exception)` to abort the forward execution and initiate the compensation (rollback) flow.
...
| `onNext()` — any exception (including `actionUtil.error(...)`)
| Compensation begins
...
NOTE: Any exception thrown from `onNext()` — or returned via `actionUtil.error(...)` — is treated by the framework as a non-retryable failure and immediately transitions the saga to `FAILED`, triggering the compensation sequence.
