...
NOTE: The `nextTopic` parameter was introduced in preparation for an upcoming sub-execution feature that will allow additional before/after sub-steps to be attached to a compensation execution. In the current version it reflects the next top-level compensation topic.
