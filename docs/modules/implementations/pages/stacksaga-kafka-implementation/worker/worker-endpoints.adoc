...
| Prefix auto-added
| The framework prepends `saga.do.` for primary topics and `saga.undo.` for compensation topics.
So `topicNameSuffix = "payment.make"` produces real topic `saga.do.payment.make`.
