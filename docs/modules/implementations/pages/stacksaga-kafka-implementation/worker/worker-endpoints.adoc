...
| Name the topic `{service-name}.{action}` after the service and action it targets — e.g. `user-service.fetch-user-details` — *not* after the saga domain, since the endpoint is a service capability that other saga domains can reuse. See the full specification linked below.
...
| Prefix auto-added
| The framework prepends `saga.do.` for primary topics and `saga.undo.` for compensation topics — you provide only the plain suffix, the *same* suffix used in the orchestrator's `AbstractTopic` definition.
So `topicNameSuffix = "payment-service.make-payment"` produces real topic `saga.do.payment-service.make-payment`.

| Topic key
| Each topic requires a unique `float` key within the domain.
Positive integers (1, 2, 3…) for primary topics; matching negative values (-1, -2, -3…) for compensation topics.
