...
<5> *listenerScope*: Provide the listener scope for the EventManager. it can be either `ListenerScope.SHARED` or `ListenerScope.ISOLATED`. see xref:#topic_model[] for more details.
...
<8> Override the `onNextRevert()` (Optional) method to perform any action before reverting the
...
. *Regular Topics* [As Consumer in `stacksaga-kafka-worker`] +
...
. *System Topics* +
.. *Shared Root Topic* [As Consumer in `stacksaga-kafka-orchestrator`] +
...
.. *Domain-Root Topics* [As Consumer in `stacksaga-kafka-orchestrator`] +
