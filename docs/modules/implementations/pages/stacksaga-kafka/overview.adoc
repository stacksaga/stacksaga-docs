...
//<2>
@SagaEventManager(
        domainRootTopicSuffix = "order-service.place-order", //<3>
        value = "placeOrderEventManager", //<4>
        listenerScope = ListenerScope.SHARED //<5>
)
...
<3> *domainRootTopicSuffix*: provide the common suffix for the topics that are related to the same domain. this is used for creating the domain-root-topic (topic for receiving the response messages from the kafka clint endpoints) internally by the framework.

<4> *value*: provide the name of the spring bean for the custom EventManager. it is used for identification of the EventManager by the name.

<5> *listenerScope*: Provide the listener scope for the EventManager. it can be either `ListenerScope.SHARED` or `ListenerScope.PROTOTYPE`
...
===== StackSaga Kafka Topic Model

StackSaga use 3 types of topics for do the orchestration as follows.

. Shared Root Topic: this topic is used for receiving the response messages from the kafka client endpoints to the respective service. all the domain that marks the `@SagaEventManager#listenerScope` as `ListenerScope.SHARED` will use the same shared root topic for receiving the response messages.
- For instance, if the service name is `order-service` and the shared root topic is created by the framework as `saga.internal.shared.order-service`.
- All the custom domains which marked as `@SagaEventManager#listenerScope=ListenerScope.SHARED` are binding to this topic for receiving the response messages from the kafka client endpoints.
- If the kafka credentials are allowed to create the topic automatically, the topic will be created by the framework at the starting if the topic is not already exist in the kafka with default partition and replication factor configurations. otherwise, you need to create the topic manually in the kafka with the name with following the convention `saga.internal.shared.{serviceName}`.
- The consumer-group is set with fallowing convention `{serviceName}-os`. the `os` stands for orchestration-service. for instance, if the service name is `order-service`, the consumer group for the shared root topic will be `order-service-os`.
service-group.
- `offset.reset` is set to `earliest` for the shared root topic to make sure that no messages are lost in case of any service downtime. and also to make sure that all the messages are consumed by the service even if there is a downtime.
- Receiving messages is configured in `at-least-once` semantics to make sure that all the messages are consumed by the service even if there is a downtime. and also to make sure that no messages are lost in case of any service downtime.
- by ensuring the `at-least-once` semantic the acknowledgement of each message is done after the processing of the message is done successfully. and acknowledged offsets are committed periodically (time-based) with default configurations.
...
. Shared Root Topic: This topic is used to consume response messages from Kafka client endpoints. All saga domains configured with `@SagaEventManager(listenerScope = ListenerScope.SHARED)` use the same shared root topic to receive response messages.
+
...
* If Kafka is configured to allow automatic topic creation, the framework creates the topic during application startup if it does not already exist. The topic is created using the framework's default partition count and replication factor configuration. Otherwise, the topic must be created manually following the naming convention: `saga.internal.shared.{serviceName}`
