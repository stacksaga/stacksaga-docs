...
*Stacksaga-Kafka Framework* is one of the StackSaga's implementations, which supports asynchronous (event-driven) communication using Kafka as the messaging backbone to handle long-running transactions (LRT) in a microservices systems.
a reactive saga orchestration framework built on top of Spring Boot, designed to manage long-running transactions using the saga pattern and Kafka for internal communication.
...
Here are some of the key advantages of using the Stacksaga-Kafka Framework over traditional Kafka-based messaging lonely:

* Centralized Transaction State Management

Just Kafka:: Just delivers messages asynchronously; it doesn’t track the state of a business transaction.
Stacksaga-Kafka Framework With Kafka:: Maintains the full transaction lifecycle: +
...
Just Kafka:: Provides message durability and delivery guarantees, but doesn’t handle transaction failures.
Stacksaga-Kafka Framework With Kafka:: Implements retry logic, timeouts, and compensation actions to recover from failures.
...
Just Kafka:: Scales well for high-throughput messaging but doesn’t optimize for transaction workflows.
Stacksaga-Kafka Framework With Kafka:: Designed to handle large volumes of transactions efficiently, leveraging Kafka’s distributed architecture.
...
Just Kafka:: Requires developers to implement saga logic and state management in each service.
Stacksaga-Kafka Framework With Kafka:: Provides a clear framework and abstractions for defining sagas, steps, and compensation actions.
+
...
Just Kafka:: Offers basic metrics and logging but lacks detailed saga tracking.
Stacksaga-Kafka Framework With Kafka:: Includes built-in monitoring tools to track saga progress, failures, and performance metrics.
+
...
The Stacksaga-Kafka Framework consists of 2 main components that work together to facilitate saga orchestration and management. the components are categorized based on their roles in the saga orchestration process.

. *stacksaga-kafka-orchestrator*
- The core component which manages the lifecycle of sagas, coordinates between services, and handles state management, compensation, etc. this is the orchestrator engine of the framework that should be added to the service that you want to make it as the orchestrator service. see more at [more]
. *stacksaga-kafka-worker*
- The component that should be added to the target services (utility services) that will participate in the sagas. it provides the necessary tools and abstractions for the target services to interact with the orchestrator service via Kafka. see more at [more]

NOTE: If a microservice acts as an orchestrator service by including the `stacksaga-kafka-orchestrator` component, other StackSaga's basic components (like `stacksaga-core`, `stacksaga-database-support`, and `stacksaga-environment-support`,`stacksaga-retry-support`) are also should be included as dependencies in the orchestrator service.
...
. Next, you have to create an enum for storing the saga steps and the topics of the respective events that should be triggered.
