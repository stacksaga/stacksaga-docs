...
Here is an example of how to create a custom `ExecutionSchedulerProvider` and configure it in the endpoint.
...
. `processingMode`
- if the `listenerScope` is set to `ListenerScope.ISOLATED`, here is where you can define the processing mode for the isolated listener. it can be one of fallowing values.
...
In most cases, a shared scheduler for all endpoints is sufficient and more efficient. but using shared default scheduler can lead to starvation of tasks.
//todo: explain about starvation of tasks when sharing the same scheduler.
