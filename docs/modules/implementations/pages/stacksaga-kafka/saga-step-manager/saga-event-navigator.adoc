...
** `Generic A`: What is the target xref:implementations:stacksaga-kafka/aggregator/create-aggregator.adoc[Custom Aggregator] class for the respective navigator.
** `Generic T`: What is the target xref:implementations:stacksaga-kafka/saga-topic/saga-topic.adoc[Custom SagaTopic] is used as the topic.
