...
. *Direct Factory Method (`SagaExecutionEventName.of`)*: +
For methods where a `SagaExecutionEventName` return type is required (such as in `doRevert(...)`) and you are not using a custom enum, you can directly use `SagaExecutionEventName.of("CANCELLED_ORDER")`.
====
+
