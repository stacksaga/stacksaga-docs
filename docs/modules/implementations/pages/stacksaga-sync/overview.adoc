...
:toc-title: Stacksaga Synchronous Engine
:robots: index,follow

= Stacksaga Synchronous Engine
...
* xref:stacksaga-sync/orchestrator/domain-entity.adoc[Domain Entity] - To store the necessary data that is required for the transaction.
...
StackSaga provides the flexibility to customize some advanced features based on your needs.

=== Saga Scheduler Configuration

Stacksaga-Framework (Synchronous) provides a default scheduler configuration for saga execution that uses Project Reactor's BoundedElastic scheduler.
This configuration is automatically applied unless you provide a custom implementation.

This scheduler is used after
