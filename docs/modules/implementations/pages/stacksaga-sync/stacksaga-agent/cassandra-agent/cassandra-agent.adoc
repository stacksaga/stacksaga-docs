...
`stacksaga-agent-cassandra-starter` is one of the xref:overview/stacksaga-agent.adoc[Stacksaga Agent implementations]  that for supporting Cassandra based Orchestrator services for retrying the transactions.
...
include::stacksaga_cassandra_agent_configuration_properties.adoc[]
