...
`stacksaga-agent-mysql` is one of the xref:overview/stacksaga-agent.adoc[Stacksaga Agent implementations] that for supporting Mysql based Orchestrator services for retrying the transactions.
...
image::mysql-agent/stacksaga-diagram-stacksaga-service-agent-mysql-how-retry-is-done.svg[]
...
image:mysql-agent/stacksaga-diagram-how-token-range-is-shared-with-agents-in-eureka-mysql.drawio.svg[alt="How token range is shared with the available agents in Eureka"]
