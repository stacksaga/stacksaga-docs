...
|`Integer.MAX_VALUE`
|`int`
|Maximum number of retry attempts when the connection to the Master succeeds but its registry currently has no Slave to serve the request. The Connector reconnects to the Master and asks for fresh Slave details on each attempt. Defaults to `Integer.MAX_VALUE` (retry indefinitely until a Slave becomes available) for maximum resilience; lower it if you would rather cap the attempts during prolonged outages.
...
|`10`
|`int`
|Maximum number of reconnection attempts to the *current* Slave connection after a connection loss. Once this limit is reached, the Connector stops retrying that Slave and looks up a fresh Slave from the Master's registry, since the original Slave may have been restarted or replaced.
