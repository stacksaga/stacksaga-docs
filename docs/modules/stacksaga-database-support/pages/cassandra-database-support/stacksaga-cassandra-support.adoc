...
See xref:retry-ring-coordinator:transaction-retrying-with-retry-coordinator.adoc#virtual-cluster[Virtual Clusters] for the full architecture and deployment topologies.
...
[.google-anno-skip]
|===
| Problem | Table Set | When a record is added / removed
...
*Removed* only when it *completes successfully*.
...
*Removed* when it is picked up for retry.
|===
...
. Retry Ring Coordinator-Based Recovery
* The xref:retry-ring-coordinator:transaction-retrying-with-retry-coordinator.adoc[Retry Ring Coordinator] collects pending transactions from the `es_rt_*` tables and distributes them to the available orchestrator instances for re-invocation.
...
In a *multi-region / multi-cluster deployment*, each Retry Ring Coordinator operates within a specific region and cluster and only accesses data within its assigned scope.
...
Retry Ring Coordinators in a specific region and cluster only access data within their scope, reducing unnecessary cross-region queries.
* *Scalability* +
...
Ideally, all transactions that complete their journey are removed from the `es_rt_*` table.
However, if even one transaction fails to complete, it persists in the table.
That transaction is then exposed to the Retry Ring Coordinator after the configured time period, which collects it from the respective `es_rt_*` table and distributes it among the available orchestrator instances for re-invocation.
This can happen due to various reasons such as system failures, network issues, or unexpected interruptions.
...
When a transaction's execution fails with a Resource Unavailability problem, the transaction is retried asynchronously by the xref:retry-ring-coordinator:transaction-retrying-with-retry-coordinator.adoc[Retry Ring Coordinator] by collecting them from the event store.
...
To overcome this,
*transaction retrying* follows the same *table-switching mechanism* used for xref:#transaction_recovery_retention[transaction recovery].

When a transaction is stopped due to resource unavailability, it is stored in one of the `es_irt_*` tables.
It will be exposed for the upcoming retry schedule by the xref:retry-ring-coordinator:transaction-retrying-with-retry-coordinator.adoc[Retry Ring Coordinator].

TIP: The key difference between *transaction recovery retention* (`es_rt_*`) and *immediate retry retention* (`es_irt_*`) is the time window: retry retention uses a much shorter delay (minutes) compared to recovery retention (hours), since paused transactions are expected to be retried quickly once the resource becomes available again.
