...
xref:#executing-schema-creation-script[Step-2]:: Execute the shema creation script that is provided by the `stacksaga-cassandra-reactive-support` module.
...
Due to the fact that StackSaga should create some dynamic tables based on the *service name* and *region* and *cluster* for saving the transaction data, the fallowing schema creation tool is provided for your convenience.
For instance, enter the service name, region, and cluster name, and then click the generate button, then the schema creation script will be generated for you based on the inputs.
you can copy or download the `.cql` file and execute it in your Cassandra database to create the necessary tables. +

IMPORTANT: If you wish to deploy the application in multiple regions or clusters, you have to execute the generated schema creation script for each region and cluster with the respective inputs.
each time the script is executed, the *Generated Tables* will be created differently based on the inputs. and the *Constant Tables* are is the same for all regions and clusters.
...
All the transactions are saved in 5 tables in the Cassandra database:
. `es_transaction` table
. `es_transaction_tryout` table
. xref:#write-protection[`es_execution_markers` table (write protection and duplicate detection)]
. `transaction_recovery_retention` tables
.. `es_{service_name}_{region_name}_transaction_recovery_retention_1` table
.. `es_{service_name}_{region_name}_transaction_recovery_retention_2` table
.. `es_{service_name}_{region_name}_transaction_recovery_retention_3` table
. `es_transaction_retry_retention` tables
.. `es_{service_name}_{region_name}_transaction_retry_retention_1` table
.. `es_{service_name}_{region_name}_transaction_retry_retention_2` table
.. `es_{service_name}_{region_name}_transaction_retry_retention_3` table
...
* `es_order_service_us_east_1_transaction_recovery_retention_1` (00:00)
* `es_order_service_us_east_1_transaction_recovery_retention_2` (08:00)
* `es_order_service_us_east_1_transaction_recovery_retention_3` (16:00)

**Tables for `us-west-1` Region** +

* `es_order_service_us_west_1_transaction_recovery_retention_1` (00:00)
* `es_order_service_us_west_1_transaction_recovery_retention_2` (08:00)
* `es_order_service_us_west_1_transaction_recovery_retention_3` (16:00)
...
. 1st schedule at: 00:00 (`es_order_service_us_transaction_retry_retention_1`)
. 2nd schedule at: 00:02 (`es_order_service_us_transaction_retry_retention_2`)
. 3rd schedule at: 00:04 (`es_order_service_us_transaction_retry_retention_3`)
. 4th schedule at: 00:06 (`es_order_service_us_transaction_retry_retention_1`)
. 5th schedule at: 00:08 (`es_order_service_us_transaction_retry_retention_2`)
. and so on...
...
=== Why Execution Markers are Needed (Kafka At-Least-Once Delivery)

StackSaga's Kafka implementation uses **at-least-once delivery semantics**. Because of this, duplicate event deliveries are naturally expected in distributed environments, for example:
...
stacksaga:
  cassandra:
    write-protection:
      default-mode: RELAXED_WITH_DEDUP      # Fallback mode for all sagas
...
4+|
*Write Protection Properties:* The following properties configure duplicate protection, LWT lease TTL durations, and per-saga consistency modes under `stacksaga.cassandra.write-protection.*`.
...
4+|
*Transaction Properties:* The following properties configure transaction lifetime, retry/restore scheduling, and the in-memory streaming buffer used by the recovery pipeline.

|`stacksaga.cassandra.transaction.lifetime`|`24h`|`Duration`|how long time the transaction keep as live. after reaching the lifetime the transaction will not be exposed for retrying even if the transaction still in processing monde. +
default- 24hours
|`stacksaga.cassandra.transaction.retry.delay-in-minutes`|`1`|`int`|How long the transaction should be kept waiting to next expose the transaction for retrying. This ensures that avoiding the transaction retrying frequently withing a short period of time. the transaction is frozen for some period of time, even the transaction is failed again after retrying recently. The value must evenly divide 1440. (24*60) the delay time can be fluctuated withing delayInMinutes to delayInMinutes+(delayInMinutes/2). for example, if the delayInMinutes is 2, the transaction can be exposed for retrying after 2 minutes to 3 minutes. +
...
default: two-minute
|`stacksaga.cassandra.transaction.restore.delay-in-minutes`|`480`|`int`|from how long time the transaction should be considered as crashed and should be restored for retrying. in rare case, the transactions may be crashed without acknowledging the failure such as application crash. in such case, the transaction is not exposed for retrying because the transaction is not updated for a long time. this value is used to determine after how long time the transaction should be considered as crashed and should be exposed for restoring the transaction and retrying. The value must evenly divide 24. the delay time can be fluctuated withing delayInMinutes to delayInMinutes+(delayInMinutes/2). for example, if the delayInMinutes is 540, the transaction can be exposed for restoring after 540 minutes to 810 minutes. +
https://docs.stacksaga.org/stacksaga/latest/stacksaga-database-support/cassandra-database-support/stacksaga-cassandra-support.html[read more] +
default: 480 minutes (8hours)
