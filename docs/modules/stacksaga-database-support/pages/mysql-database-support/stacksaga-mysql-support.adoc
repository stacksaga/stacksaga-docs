...
:db_engine: MySQL
:db_path: mysql
:autoconfig_class: StackSagaMysqlAutoConfiguration
...
[[mysql-partitioning-maintenance]]
== Table Partition Maintenance

The MySQL event-store tables (`es_transaction` and `es_transaction_execution_tryout`) use range partitioning based on `created_at` to ensure fast inserts and index performance.

To automate the creation of upcoming daily partitions without coupling DDL execution to your application microservice instances, refer to the dedicated xref:sql-database-support/stacksaga-sql-partition-support.adoc[StackSaga SQL Partition Support] starter. It can be deployed once centrally to manage partition creation across all your MySQL and PostgreSQL databases.
